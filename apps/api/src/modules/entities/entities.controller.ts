import { Body, Controller, Delete, ForbiddenException, Get, NotFoundException, Param, Patch, Post, Query, UnauthorizedException, UseGuards } from '@nestjs/common';
import { EntityName, PUBLIC_READ_ENTITIES } from '../../common/constants/entity-names';
...
export class EntitiesController {
  constructor(
    ...
    private readonly mutations: EntityMutationOrchestratorService,
  ) {}
  ...
  async list(
    ...
    return this.entityRepository.list(entityName, context, sort, limit ? Number(limit) : undefined);
  }
  ...
  async create(
    ...
    if (Array.isArray(body)) {
      return this.mutations.bulkCreate(entityName, body, context);
    }

    return this.mutations.create(entityName, body || {}, context);
  }
  ...
  async update(
    ...
    const entityName = this.ensureEntity(entity);
    const context = await this.entityAccess.createContext(user.sub, user.role);
    return this.mutations.update(entityName, id, body || {}, context);
  }
