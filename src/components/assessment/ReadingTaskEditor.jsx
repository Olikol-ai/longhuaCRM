...
export default function ReadingTaskEditor({ open, onOpenChange, editing = null, onSaved }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        if (editing?.id) {
          const detail = await api.assessment.getReadingTask(editing.id);
  ...
  const handleSave = async () => {
    ...
    if (!textContent.trim()) {
      setError('Укажите текст для чтения');
      return;
    }
    ...
    try {
      const saved = editing?.id
        ? await api.assessment.updateReadingTask(editing.id, payload)
        : await api.assessment.createReadingTask(payload);
      onSaved?.(saved);
      onOpenChange(false);
    } catch (err) {
