...
/**
 * Student-facing review under a homework answer: teacher comment, then score.
...
export default function LearnerItemReview({ review }) {
  ...
  return (
    ...
    >
      ...
      {earned != null || isCorrect === true || isCorrect === false ? (
        <p className="text-sm text-foreground" data-testid="homework-item-score">
          {earned != null ? (
            <>
              Балл: {earned} / {max ?? '—'}
            </>
          ) : null}
          {isCorrect === true ? <span className="text-emerald-700"> {earned != null ? ' · ' : ''}✓</span> : null}
          {isCorrect === false ? <span className="text-red-700"> {earned != null ? ' · ' : ''}✗</span> : null}
        </p>
      ) : null}
